...
Quantizing \texttt{mistralai/Mistral-7B-Instruct-v0.2} to 4-bit increases refusals on jailbreak prompts that it previously answered under FP16. This pattern is opposite to our initial hypothesis that quantization would reduce safety alignment. In our synthetic jailbreak subset (200 prompts), \texttt{mistralai/Mistral-7B-Instruct-v0.2} complied with all jailbreak prompts at FP16 but refused a meaningful subset at INT4.
...
Substring matching frequently misses nuanced refusals. Therefore, outputs are evaluated using a robust Llama-3-70B semantic classifier prompted to label responses as Safe (Refusal) or Unsafe (Compliance). We also performed a small manual spot-check to verify that judge labels were directionally consistent with human assessment.
...
\caption{Violin distributions of log-probability refusal margins by precision level. \texttt{mistralai/Mistral-7B-Instruct-v0.2} shows the largest distributional shift at INT4, with visibly wider spread.}
\label{fig:violin}
\end{figure}

Critically, after Benjamini--Hochberg correction across the model/precision family of comparisons, the strongest observed signal (\texttt{mistralai/Mistral-7B-Instruct-v0.2} INT4 vs FP16) does not remain statistically significant (adjusted $p=0.16$). We therefore interpret the pattern as exploratory evidence and not a confirmatory finding.
For completeness, the same \texttt{mistralai/Mistral-7B-Instruct-v0.2} INT4-vs-FP16 comparison yields an uncorrected odds ratio of $OR=1.46$, indicating higher refusal odds in the INT4 condition prior to multiple-comparison correction.

INT8 results are comparatively stable in this benchmark (Table~\ref{tab:refusal}), with non-significant drift p-values across models (\texttt{google/gemma-2b-it}: $p=0.365$, \texttt{mistralai/Mistral-7B-Instruct-v0.2}: $p=0.108$, \texttt{meta-llama/Meta-Llama-3-8B-Instruct}: $p=0.773$).
...
As shown, general capabilities experience a steady decay of roughly 2.4\% on MMLU while refusal behavior jumps by over 5.3\% absolute for \texttt{mistralai/Mistral-7B-Instruct-v0.2}. This decoupling demonstrates that alignment drift is not merely a side effect of generalized intelligence loss under compression.

\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth]{safety_utility_tradeoff.pdf}
\caption{Safety-utility tradeoff view for the \texttt{mistralai/Mistral-7B-Instruct-v0.2} capability-control slice (FP16/INT8/INT4). Utility is summarized as the mean of MMLU and GSM8K, while safety is represented by refusal rate. The trend illustrates increasing refusal as utility decreases under stronger quantization.}
...
This study has several limitations. First, only one of the three models (\texttt{mistralai/Mistral-7B-Instruct-v0.2}) showed a notable effect; \texttt{google/gemma-2b-it} and \texttt{meta-llama/Meta-Llama-3-8B-Instruct} did not, so we cannot claim universality. Second, our judge model (Llama-3-70B) may introduce its own alignment biases; with additional compute, we would evaluate it as both judge and subject to test this sensitivity directly. Third, we did not run layer-wise ablations to localize the mechanism of the observed shift, which remains future work. Finally, we performed multiple significance tests across models and quantization levels, so isolated uncorrected p-values should be interpreted cautiously.
