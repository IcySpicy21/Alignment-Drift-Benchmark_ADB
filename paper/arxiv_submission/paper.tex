...
Quantizing mistralai/Mistral-7B-Instruct-v0.2 to 4-bit increases refusals on jailbreak prompts that it previously answered under FP16. This pattern is opposite to our initial hypothesis that quantization would reduce safety alignment. In our synthetic jailbreak subset (200 prompts), mistralai/Mistral-7B-Instruct-v0.2 complied with all jailbreak prompts at FP16 but refused a meaningful subset at INT4.
...
Critically, after Benjamini--Hochberg correction across the model/precision family of comparisons, the strongest observed signal (mistralai/Mistral-7B-Instruct-v0.2 INT4 vs FP16) does not remain statistically significant (adjusted $p=0.16$). We therefore interpret the pattern as exploratory evidence and not a confirmatory finding.
For completeness, the same mistralai/Mistral-7B-Instruct-v0.2 INT4-vs-FP16 comparison yields an uncorrected odds ratio of $OR=1.46$, indicating higher refusal odds in the INT4 condition prior to multiple-comparison correction.

INT8 results are comparatively stable in this benchmark (Table~\ref{tab:refusal}), with non-significant drift p-values across models (google/gemma-2b-it: $p=0.365$, mistralai/Mistral-7B-Instruct-v0.2: $p=0.108$, meta-llama/Meta-Llama-3-8B-Instruct: $p=0.773$).
...
As shown, general capabilities experience a steady decay of roughly 2.4\% on MMLU while refusal behavior jumps by over 5.3\% absolute for mistralai/Mistral-7B-Instruct-v0.2. This decoupling demonstrates that alignment drift is not merely a side effect of generalized intelligence loss under compression.

\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth]{safety_utility_tradeoff.pdf}
\caption{Safety-utility tradeoff view for the mistralai/Mistral-7B-Instruct-v0.2 capability-control slice (FP16/INT8/INT4). Utility is summarized as the mean of MMLU and GSM8K, while safety is represented by refusal rate. The trend illustrates increasing refusal as utility decreases under stronger quantization.}
...
Table~\ref{tab:category} shows a category-specific shift in jailbreak behavior for mistralai/Mistral-7B-Instruct-v0.2 under INT4. At FP16, mistralai/Mistral-7B-Instruct-v0.2 complies with jailbreak overrides; at INT4, we observe increased refusal on the same category. We interpret this as evidence of potential safety-boundary drift rather than definitive proof of a robust defense mechanism.
...
This study has several limitations. First, only one of the three models (mistralai/Mistral-7B-Instruct-v0.2) showed a notable effect; google/gemma-2b-it and meta-llama/Meta-Llama-3-8B-Instruct did not, so we cannot claim universality. Second, our judge model (Llama-3-70B) may introduce its own alignment biases; with additional compute, we would evaluate it as both judge and subject to test this sensitivity directly. Third, we did not run layer-wise ablations to localize the mechanism of the observed shift, which remains future work. Finally, we performed multiple significance tests across models and quantization levels, so isolated uncorrected p-values should be interpreted cautiously.

\subsection{Planned Replication Experiments}
To convert this exploratory signal into a confirmatory claim, the next iteration of this benchmark will prioritize: (1) broader model coverage (including additional open families), (2) multi-seed evaluation to estimate variance and confidence intervals more robustly, (3) quantization-method robustness checks across toolchains/settings, and (4) expanded human-vs-judge agreement auditing. These additions are designed to test whether the mistralai/Mistral-7B-Instruct-v0.2 INT4 pattern is a stable phenomenon or a benchmark-specific artifact.
...
Quantization introduces model-family-specific behavioral shifts. Our findings suggest that INT4 compression can be associated with over-refusal in susceptible architectures (mistralai/Mistral-7B-Instruct-v0.2) while leaving others (google/gemma-2b-it, meta-llama/Meta-Llama-3-8B-Instruct) comparatively stable in this benchmark. However, over-refusal is also a production concern: excessive false positives degrade user experience, reduce utility, and erode trust in AI assistants. After FDR correction, the mistralai/Mistral-7B-Instruct-v0.2 signal (adjusted p =0.16) should be interpreted as exploratory evidence motivating replication, not as definitive proof.
...
\bottomrule
\end{tabular}
\caption{Layer-wise quantization ablation on mistralai/Mistral-7B-Instruct-v0.2. MLP layers account for $\sim$81\% of the total alignment drift, suggesting that the feed-forward projections are the primary locus of safety-critical weight interactions disrupted by 4-bit precision noise.}
\label{tab:layerwise}
\end{table}
